...
*   supplied, and under no circumstances can it be used with non-TI
*   connectivity device.
* ==================================================================== */
...
static struct wfd_device_info_desc *wfd_find_device_info(struct wfd_data *wfd,
					const u8 *addr)
{
	struct wfd_device_info_desc *dev, *n;

	dl_list_for_each_safe(dev, n, &wfd->devices,
				struct wfd_device_info_desc, list) {
		if (os_memcmp(dev->dev_addr, addr, ETH_ALEN) == 0) {
			/* Device found */
			wpa_printf(MSG_DEBUG, "%s: found %p\n", __func__, dev);
			return dev;
		}
	}
	return NULL;
...
void wfd_build_go_neg_resp_ie(struct wfd_data *wfd, struct wpabuf *buf)
{
	u8 *len;
...
void wfd_build_go_neg_conf(struct wfd_data *wfd, struct wpabuf *buf)
{
	u8 *len;
...
void wfd_build_invitation_req_ie(struct wfd_data *wfd, struct wpabuf *buf)
{
	u8 *len;
...
void wfd_build_invitation_resp_ie(struct wfd_data *wfd, struct wpabuf *buf)
{
	u8 *len;
...
void wfd_build_prov_disc_resp_ie(struct wfd_data *wfd, struct wpabuf *buf)
{
	u8 *len;
...
int wfd_build_assoc_req_ie(struct wfd_data *wfd, size_t space, u8 *buf)
{
	struct wpabuf *tmp;
	size_t tmplen;
	u8 *len;
	...
	if (wfd->enabled) {
		...
		len = wfd_buf_add_ie_hdr(tmp);
		wfd_buf_add_device_info(wfd, tmp);
		if (wfd->associated)
			wfd_buf_add_assoc_bssid(wfd, tmp);
		wfd_buf_add_coupled_sink(wfd, tmp);
		wfd_add_session_info(wfd, tmp);
		wfd_buf_update_ie_len(tmp, len);
...
struct wpabuf *wfd_build_assoc_resp_ie(struct wfd_data *wfd)
{
	u8 *len;
